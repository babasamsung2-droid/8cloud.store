      ...
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/10 transition-all duration-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* Admin Panel Quick Access - ONLY visible when Baba Samsung admin is logged in */}
            {isBabaAdmin && onOpenAdmin && (
              <button
                onClick={onOpenAdmin}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-500/20 to-amber-600/20 hover:from-amber-500/30 text-amber-300 border border-amber-500/40 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-md shadow-amber-500/10"
                title="Store Administrator Panel (Full Access)"
              >
            ...
            {currentUser ? (
              <button
                onClick={onOpenVault}
                className="flex items-center gap-1.5 px-2.5 py-1.5 bg-zinc-900/80 hover:bg-zinc-800 border border-cyan-500/40 text-white rounded-xl text-xs font-medium transition-colors cursor-pointer shadow-sm shadow-cyan-500/10"
              ...
              <button
                onClick={onOpenLogin}
                className="flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r from-cyan-500/20 via-zinc-900 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 text-white border border-cyan-400/40 hover:border-cyan-400 rounded-xl text-xs font-bold transition-all shadow-md shadow-cyan-500/10 active:scale-95 cursor-pointer"
                title="Sign In with ID & Password or 1-Click Gmail"
              >
                {/* Google G mini-badge */}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-white/10 bg-zinc-950/95 backdrop-blur-xl px-4 py-4 space-y-3">
            ...
            {/* Admin Panel for Mobile - ONLY if Baba Samsung is logged in */}
            {isBabaAdmin && onOpenAdmin && (
              <button
            ...
            <div className="pt-2 border-t border-white/5 flex items-center justify-between text-xs text-zinc-400">
              {currentUser ? (
                ...
                <button 
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-cyan-500/20 via-zinc-900 to-blue-500/20 text-white font-bold text-xs flex items-center gap-2 border border-cyan-400/40 cursor-pointer active:scale-95 shadow-sm"
                >
                  <div className="w-4 h-4 rounded-full bg-white flex items-center justify-center shrink-0">
                    <svg className="w-2.5 h-2.5" viewBox="0 0 24 24">
                      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
